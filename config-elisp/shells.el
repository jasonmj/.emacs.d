...
(use-package vterm-toggle
  ...
  (add-hook 'vterm-toggle-show-hook
            (lambda ()
              (setq cursor-type 'hbar)
              (setq-default cursor-type 'hbar)))
  (emacs-set-key (kbd "M-s-t") 'vterm-toggle))

(defun ghostel-toggle-or-create ()
  "Toggle ghostel visibility or create a new instance if none exists.
Check for any buffer starting with \"*ghostel:\" and whether it's currently displayed."
  (interactive)
  (let* ((ghostel-buf (seq-find (lambda (buf) (string-prefix-p "*ghostel:" (buffer-name buf)))
                                 (buffer-list)))
         (ghostel-window (when ghostel-buf (get-buffer-window ghostel-buf))))
    (if (and ghostel-buf ghostel-window)
        ;; Ghostel buffer exists and is visible in a window - hide it
        (if (> (length (window-list)) 1)
            ;; Multiple windows - delete the ghostel window
            (delete-window ghostel-window)
          ;; Sole window - bury the buffer to switch to another one
          (with-current-buffer ghostel-buf
            (bury-buffer)))
      ;; Either no ghostel buffer exists, or it exists but is not visible - show/create it
      (if ghostel-buf
          (pop-to-buffer ghostel-buf)
        (ghostel)))))
(use-package ghostel
  :ensure t
  :bind (
         ("C-s-t" . ghostel-toggle-or-create)
